...
| `release-benchmark` | 10 | PR coverage plus packaging and hostile-archive preflight |
| `full` | 40 | Complete behavior catalog |

The exact PR case IDs are `101, 104, 106, 108, 109, 114, 139, 140`. The release
suite adds `110` and `127`. Cases `139` and `140` retain the supplemental
`requirements-discovery` and `multi-turn` tags. `critical_expectation_indexes` and
